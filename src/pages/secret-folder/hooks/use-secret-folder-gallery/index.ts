...
import { type SecretFolderAsset, assetsAtom, assetsStore } from './atom';
...
export function useSecretFolderGallery({
  ...
  const addAssets = useCallback(
    async (limit?: number) => {
      let error: 'limit exceeded' | 'no assets picked' | null = null;
      if (limit === 0) {
      ...
      const pickedAssets = await (selectedOption === 'camera'
        ? openCameraPicker
        : openGalleryPicker)();

      if (!pickedAssets.length) {
        error = 'no assets picked';
        return { data: [], error };
      }

      if (limit !== undefined && pickedAssets.length > limit) {
        error = 'limit exceeded';
      }
      ...
      const storedAssets = await Promise.all(
        allowedPhotos.map(async ({ tempUri, ...item }) => {
          ...
          const originalUri = newImagePath.replace(FileSystem.documentDirectory ?? '', '');
          ...
          return {
            ...
            uri: (item.mediaType === 'video'
              ? await (async () => {
                  ...
                  await CameraRoll.extractThumbnail(item.id, videoPreviewPath, -1, -1);
      ...
      return { data: storedAssets, error };
    },
    [
      setAssets,
      openGalleryPicker,
  ...
  const deleteAssets = useCallback(
    async (ids: string[]) => {
      ...
      // console.log('restoreFiles', restoreFiles);
      if (restoreFiles) {
      ...
      await Promise.all([
        ...
        ...assetsToDelete
          .filter((x: SecretFolderAsset) => x.thumbnailUri && x.thumbnailUri !== x.uri)
          .map((x: SecretFolderAsset) =>
            FileSystem.deleteAsync(`${FileSystem.documentDirectory}${x.thumbnailUri}`)
          ),
      ]).catch((err) => {
        console.error(err);
      });
...
function defaultRemoveAfterImportPrompt() {
  return new Promise<boolean>((resolve) => {
    Alert.alert(
      ...
      'Do you want to remove imported files from your gallery?',
      [
        {
          text: 'No',
