...
import { type Storage, initialStorageState } from '@/config/storage';
...
const createAtom = (key: string, initialValue: any) => {
  ...
  const storageAtom = atom(
    storedValue ? (JSON.parse(storedValue) as typeof initialValue) : initialValue
  );

  return atom(
    (get) => get(storageAtom),
    (get, set, newValue) => {
      const value = typeof newValue === 'function' ? newValue(get(storageAtom)) : newValue;
